import torch
import numpy as np
from helper import dump_compressed_pickle_file, load_compressed_pickle_file
...
class Agent_NCA(BaseAgent):
    ...
    def make_seed(self, img):
        ...
        if (self.exp.dataset.slice != None or self.exp.get_from_config('2D') == True):
            if len(img.shape) == 3:
                seed = torch.zeros((img.shape[0], img.shape[1], img.shape[2], self.exp.get_from_config('channel_n')),
                                   dtype=torch.float32,
                                   device=self.device)  # torch.from_numpy(np.zeros([img.shape[0], img.shape[1], img.shape[2], self.exp.get_from_config('channel_n')], np.float32)).to(self.device)
            ...
            if len(img.shape) == 4:
                seed = torch.zeros(
                    (img.shape[0], img.shape[1], img.shape[2], img.shape[3], self.exp.get_from_config('channel_n')),
                    dtype=torch.float32,
                    device=self.device)  # torch.from_numpy(np.zeros([img.shape[0], img.shape[1], img.shape[2], self.exp.get_from_config('channel_n')], np.float32)).to(self.device)
                seed[..., 0] = img
            else:
    ...
    def repeatBatch(self, seed, target, batch_duplication):
        ...
        """
        return torch.Tensor.repeat_interleave(seed, batch_duplication, dim=0), torch.Tensor.repeat_interleave(target,
                                                                                                              batch_duplication,
                                                                                                              dim=0)
    ...
    def get_outputs(self, data, full_img=False, **kwargs):
        ...
        """
        id, inputs, targets = data['id'], data['image'], data['label']
        outputs = self.model(inputs, steps=self.getInferenceSteps(),
