...
class BasicNCA3D(nn.Module):
    def __init__(self, channel_n, fire_rate, device, hidden_size=128, input_channels=1, init_method="standard",
                 kernel_size=7, groups=False, **kwargs):
        r"""Init function
            #Args:
        ...
        self.p0 = nn.Conv3d(channel_n, channel_n, kernel_size=kernel_size, stride=1, padding=self.padding,
...
class BasicNCA3D_CL_simple(BasicNCA3D):
    def __init__(self, *args, **kwargs):
        ...
        self.adjust_0 = nn.Conv3d(2 * self.channel_n, 2 * self.channel_n, kernel_size=1, stride=1, padding=0,
                                  groups=2 * self.channel_n)
        self.adjust_1 = nn.Conv3d(self.hidden_size, self.hidden_size, kernel_size=1, stride=1, padding=0,
...
class BasicNCA3D_CL_two_adjusts(BasicNCA3D):
    ...
    def _new_adjust(self):
        self.adjusts[str(self.register)] = nn.ModuleDict({
            "adjust_0": nn.Conv3d(2 * self.channel_n, 2 * self.channel_n, kernel_size=1, stride=1, padding=0,
                                  groups=2 * self.channel_n),
            "adjust_1": nn.Conv3d(self.hidden_size, self.hidden_size, kernel_size=1, stride=1, padding=0,
                                  groups=self.hidden_size)
        })
        self.register += 1
...
class BasicNCA3D_CL_two_adjusts_and_conv_adjust(BasicNCA3D):
    ...
    def _new_adjust(self):
        ...
        self.p0s[str(self.register)] = nn.Conv3d(self.channel_n, self.channel_n, kernel_size=self.kernel_size, stride=1,
                                                 padding=self.padding, padding_mode="reflect", groups=self.channel_n)
        self.register += 1
