...
def visualize_all_channels_fast(img, replace_firstImage=None, min=1, max=100, labels=None):
    ...
    img_all_channels_red[img_all_channels_red > min] = np.log(
        img_all_channels_red[img_all_channels_red > min]) / np.log(max) + 0.5

    img_all_channels_green[img_all_channels_green <= min] = (img_all_channels_green[
                                                                 img_all_channels_green <= min] / min) * 0.5
    img_all_channels_green[img_all_channels_green > min] = np.log(
        img_all_channels_green[img_all_channels_green > min]) / np.log(max) + 0.5
...
def visualize_all_channels(img, replace_firstImage=None, divide_by=3, labels=None, color_map="nipy_spectral",
                           size=(1700, 1700)):
    ...
    pos = axes.imshow(img_all_channels, norm=colors.SymLogNorm(linthresh=0.3, linscale=0.3,
                                                               vmin=-10.0, vmax=10.0),
                      cmap=color_map)  # cmap='RdBu', aspect='auto', vmin=-100, vmax=100)
