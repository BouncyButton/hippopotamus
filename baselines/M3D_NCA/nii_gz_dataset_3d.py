...
import cv2
import random
import torch, torchio, torchvision


class Dataset_NiiGz_3D(Dataset_3D):
    """This dataset is used for all NiiGz 3D nnUnet_raw. It can handle 3D data on its own, but is also able to split them into slices. """
    ...
    def __getitem__(self, idx: str) -> tuple:
        ...
        if not img:
            ...
            img, label = self.load_item(os.path.join(self.images_path, img_name)), self.load_item(
                os.path.join(self.labels_path, img_name))
            ...
            # 3D
                ...
                if self.exp.get_from_config('rescale') is not None and self.exp.get_from_config('rescale') is True:
                    img, label = self.rescale3d(img), self.rescale3d(label, isLabel=True)
                if self.exp.get_from_config('keep_original_scale') is not None and self.exp.get_from_config(
                        'keep_original_scale'):
        ...
        if self.exp.get_from_config('patchify') is not None and self.exp.get_from_config(
                'patchify') is True and self.state == "train":
        ...
        # # CENTER CROP IT -- TEMPORARY AND ONLY FOR CARDIAC DATA IN COMBINATION WITH NCAdapt
        shape = img.shape
        # startx = shape[0]//2-(112//2)
        # starty = shape[1]//2-(112//2)
        # img = img[startx:startx+112, starty:starty+112, :]
        # label = label[startx:startx+112, starty:starty+112, :, :]

        # ONLY FOR UNETS WHEN TRAINING ON CARDIAC 89, FOR 88 ONLY FOR INFERENCE or TRANSUNET REMOVE
        # transform = torchio.CropOrPad((shape[0], shape[1], shape[2]+4))
        # img = transform(img[np.newaxis, ...])[0]
        # label = transform(label[np.newaxis, ...][-1])[0][..., np.newaxis]
